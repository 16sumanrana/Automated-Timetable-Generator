...
<html>
<head>
    <title>Generate Table | Automated Timetable Generator</title>
    <meta charset="utf-8">
...
<section>
    <div class="container" style="text-align: center;">
        ...
        <br>
        <div class="btn-group">
            <form onsubmit="return startGeneration();">
                <input type="hidden" value="generate" name="isGenerate">
                <input class="btn btn-dark" type="submit" value="Generate">
            </form>
        </div>
        <div class="btn-group">
            <form onsubmit="return getStatus();">
                <input class="btn btn-dark" type="submit" value="Get Status">
            </form>
        </div>
        <ul id="generation">
...
<script>
    ...
    function startGeneration() {
        if (isRunning === true) {
            $('#generation').append("<li class=\"status\">" + "Already running" + "</li>");
            return false;
        } else {
            $.ajax({
                method: "post",
                url: "<%out.print(Config.BASEURL);%>/Main",
                beforeSend: function () {
                    isRunning = true;
                    var dt = new Date();
                    $('#generation').append("<li class=\"status\">" + dt + "</li>");
                },
                data: {
                    isGenerate: true
                },
                error: function (jqXHR, exception) {
                    ...
                    $('#generation').append("<li class=\"status\">" + msg + "</li>");
                }
            }).done(function (msg) {
                var dt = new Date();
                $('#generation').append("<li class=\"status\">" + dt + "</li>");
    ...
    function getStatus() {
        $.ajax({
            method: "post",
            url: "<%out.print(Config.BASEURL);%>/Status",
            error: function (jqXHR, exception) {
                var msg = '';
                if (jqXHR.status === 0) {
                    msg = 'Not connect.\n Verify Network.';
            ...
            if (msg["status"] === true) {
                $("#generation").append("<li class=\"status\">" + msg["statusText"] + "</li>");
            } else {
                $("#generation").append("<li class=\"status\">" + msg["statusText"] + "</li>");
            }
        });
